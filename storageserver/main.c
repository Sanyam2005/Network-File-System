...
void *handleClientRequest(void *client_sock) {
    ...
    while ((read_size = recv(client_socket, &Packet, sizeof(Packet), 0)) > 0) {
        if (Packet.type == 1) {
            ...
            strcpy(comparestr, Packet.data);
            if (strstr(comparestr, ".txt") == NULL && strstr(comparestr, ".c") == NULL && strstr(comparestr, ".mp3") == NULL&&strstr(comparestr, ".h") == NULL)
            {
        ...
        } else if (Packet.type == 1010) {
            printf("Received STREAM request for file: %s\n", Packet.data);
            ErrorCode res2;
            char resolved_path[4096];
            if (realpath(Packet.data, resolved_path) == NULL) {
                perror("Path resolution failed");
...
int main() {
    ...
    char* selected_paths[4096];
    int selected_count = 0;
    char input[10];
    FILE *file = fopen("ss9_paths.md", "r");
    if (file == NULL) {
    ...
    while (fgets(line, sizeof(line), file) != NULL) {
        ...
        line[strcspn(line, "\n")] = '\0';
        selected_paths[selected_count] = malloc(strlen(line) + 5);
        strcpy(selected_paths[selected_count++],line);
    }
    ...
    for (int i = 0; i < selected_count; i++) {
        strcat(selected_file_paths, selected_paths[i]);
