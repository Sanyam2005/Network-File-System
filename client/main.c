...
{   
    ...
    if(cli_sock == -1)
    {
        printf("Connection failed : Storage server down\n");
        return;
    }
    packet req;
    req.type = type;
    ...
    if (type == 1) {
        ...
        if (pid < 0) {
            ...
            ssize_t size_received = recv(cli_sock, &file_size, sizeof(file_size), 0);
            if (size_received <= 0) {
            ...
            while ((bytes_received = recv(cli_sock, buffer, sizeof(buffer), 0)) > 0) {
                total_received += bytes_received;
                fwrite(buffer, 1, bytes_received, audio_player);
                fflush(audio_player);

                // Stop if all bytes are received
...
int main(){
    ...
    while(1){
        ...
        if (strcmp(token, "READ") == 0) {
            ...
            printf("HERE %s %s\n", ipaddress ,port);
            close(client_sock);
            communicate_with_ss(ipaddress, port, file_path, STREAM);
