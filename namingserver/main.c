...
void *handle_client_operations(void *socket_desc) {
    ...
    while ((read_size = recv(client_socket, &Packet, sizeof(Packet), 0)) > 0) {
        ...
        Log(0, -1, client_socket, Packet.type, Packet.data, -1, "");
        if (Packet.type == 1 || Packet.type == 2 || Packet.type == 3 || Packet.type == 1010) {
            ...
            Log(0, -1, client_socket, response_packet.type, response_packet.data, res2, "");
            pthread_mutex_unlock(&lock);
        }
        if (Packet.type == 4 || Packet.type == 5) {
            ...
            if(file_found){
                ...
                if(ss_socket==-1){
                    printf("Connection failed : Storage server failed\n");
                    packet response_packet_cl;
                    response_packet_cl.type = INVALID_SS;
                    if (send(client_socket, &response_packet_cl, sizeof(response_packet_cl), 0) < 0) {
                        perror("Send failed");
                    }
                    Log(0, -1, client_socket, response_packet_cl.type, response_packet_cl.data, res2, "");
                    continue;
                ...
                close(ss_socket);
                Log(1, temp.idx, temp.port_nm, response_packet.type, "", res2, temp.ip);
            }
            else{
                res2 = ERR_NOT_FOUND_FILE;
            }
        ...
        if(Packet.type==8){
           ...
           char total_buffer[BUFFER_SIZE];
            ...
            Log(0, -1, client_socket, data_packet.type, data_packet.data, 100, "");
            ...
            Log(0, -1, client_socket, response_packet_cl.type, response_packet_cl.data, 100, "");
        }
                ...
                // }
                 if (strstr(src, ".txt") != NULL || strstr(src, ".c") != NULL || strstr(src ,".mp3") != NULL||strstr(src, ".h") != NULL) {
                   ...
                   // printf("%s %d\n",response_packet.data,response_packet.type);
                    ...
                    //copy folder
                     const char *base = strrchr(src, '/'); // Find the last '/'
                    if (base == NULL) {
                        base = src; // If no '/', the entire src is the base
                    } else {
                        base++; // Skip the '/'
                    }   
                    printf("Base: %s\n", base);  
                    ...
                    for (int i = 0; i < cnt; i++) {
                        ...
                        if (strstr(paths[i], ".txt") != NULL || strstr(paths[i], ".c") != NULL || strstr(paths[i], ".mp3") != NULL||strstr(paths[i], ".h") != NULL) {
                           ...
                           // printf("%s %d\n",response_packet.data,response_packet.type);
                            ...
                            char new_absolute_path[4094];
                            // Construct the new absolute path: dest + "/" + base
                            snprintf(new_absolute_path, 4096, "%s/%s", dest, base);
                            char *remaining_path = NULL; // Point to the null terminator
                            char *subdir_start = strstr(path, base);   // Find where base starts in src
                            printf("Subdir start: %s\n", subdir_start);
                            if (subdir_start) {
                                remaining_path = subdir_start + strlen(base); // Skip the base part
                            }
                            if (*remaining_path == '/') { // Skip leading '/'
                                if (strcmp(remaining_path, "/") == 0) { // Check if remaining_path is not just "/"
                                    remaining_path++;
                                }
                            }
                            strcat(new_absolute_path, remaining_path);
                            strcat(response_packet.data, new_absolute_path);
                            
                            // char *last_token = strstr(path, lastToken);
                            // if (last_token != NULL) {
                            //     path = last_token; // Move the path to start from "lasttoken"
                            // }
                            //strcat(response_packet.data, path);
                            printf("ANSWER2***   : %s\n", response_packet.data);
                           // strcat(response_packet.data, path);
                            strcpy(arr5,new_absolute_path);
                           // printf("Response packet type: %s\n", response_packet.data);
                                if (send(dest_socket, &response_packet, sizeof(response_packet), 0) < 0) {
                                    perror("Send failed");
                                    res2=ERR_OPERATION_FAILURE_PASTE;
                           ...
                           char *path = paths[i];
                            ...
                            // Construct the new absolute path: dest + "/" + base
                            snprintf(new_absolute_path, 4096, "%s/%s", dest, base);
                            printf("New absolute path: %s\n", new_absolute_path);
                            printf("dest: %s\n", dest);
                            // Construct the relative path: base + remaining path
                            char *remaining_path = NULL; // Point to the null terminator
                            char *subdir_start = strstr(path, base);   // Find where base starts in src
                            printf("Subdir start: %s\n", subdir_start);
                            if (subdir_start) {
                            ...
                            //snprintf(new_absolute_path, 4096, "%s/%s", new_absolute_path, remaining_path);
                            //printf("New absolute path: %s\n", new_absolute_path);
                            strcpy(response_packet.data, new_absolute_path);
                            strcat(response_packet.data, remaining_path);
